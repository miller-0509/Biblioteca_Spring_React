...
> Nota: el proyecto se llama **Biblioteca_Spring_React**. Antes conservaba el nombre histórico `fincas-api` (carpeta, artifactId y paquete `co.sena.adso.fincasapi`), ya renombrado. La base de datos conserva el nombre `adso_fincas`. Se replica el sistema Flask de referencia (`Proyecto_gestion_biblioteca`).
